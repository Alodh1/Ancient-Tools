            ...
            if (Api.Side == EnumAppSide.Client)
            {
                if (AttachedEntity != null)
                {
                    if (EntityTransform.DistanceTo(AttachedEntity.Pos) <= 20.0)
                        SetLookAtVector(EntityTransform.XYZFloat, AttachedEntity.Pos.XYZFloat);
                    else
                    {
        ...
        public override void OnInteract(EntityAgent byEntity, ItemSlot itemslot, Vec3d hitPosition, EnumInteractMode mode)
        {
            ...
            else if (mode == EnumInteractMode.Attack)
            {
                if (CanAccessInClaim(byEntity as EntityPlayer, EnumBlockAccessFlags.BuildOrBreak))
                {
                    ...
                    int damagetier = itemslot.Itemstack == null ? 0 : itemslot.Itemstack.Collectible.GetToolTier(itemslot);
